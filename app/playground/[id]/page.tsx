...
const MainPlaygroundPage: React.FC = () => {
  ...
  const {
    ...
    instance,
    writeFileSync,
    // @ts-expect-error: instance might be undefined initially
  ...
  const handleSave = useCallback(
    async (fileId?: string) => {
      ...
      try {
        ...
        const updateFileContent = (items: Record<string, unknown>[]): unknown[] =>
          items.map((item) => {
            if ("folderName" in item) {
              return { ...item, items: updateFileContent(item.items as Record<string, unknown>[]) };
            } else if (
